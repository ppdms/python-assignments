...
def board(n):
    ...
    return {x: False for x in [(x, y) for x in range(n) for y in range(n)]}
...
def get_size(board):
    ...
    return max([x[0] for x in board.keys()])+1 if max([x[0] for x in board.keys()])==max([x[1] for x in board.keys()]) else None
...
def copy_board(board):
    ...
    return {x: board[x] for x in board.keys()}
...
def get_iterator(board):
    ...
    return iter([(x, board[x]) for x in sorted(list(board.keys()))])
...
def print_board(board):
    ...
    print(reduce(lambda a, b: str(a)+"\n"+str(b), [reduce(lambda a,b: a+b, ["⬛" if board[x] else "⬜" for x in sorted(list(board.keys()))][i*get_size(board):(i+1)*get_size(board)]) for i in range(get_size(board))]))
...
def neighbors(p):
    ...
    return set(filter(lambda x: x!=p, [(x,y) for x in [p[0]-1, p[0], p[0]+1] for y in [p[1]-1, p[1], p[1]+1]]))
...
def place_blinker(board, p = (0,0)):
    ...
    for x in board:
        if x in [(p[0], p[1]), (p[0]+1, p[1]), (p[0]+2, p[1])]:
            board[x] = True
...
def tick(board):
    ...
    def neighbors(board, p):
        return filter(lambda x: True if x in board.keys() else False, filter(lambda x: x!=p, [(x,y) for x in [p[0]-1, p[0], p[0]+1] for y in [p[1]-1, p[1], p[1]+1]]))

    next = copy_board(board)

    for x in board:
        if is_alive(board, x):
            if reduce(lambda a, b: a+b, [1 if is_alive(board, x) else 0 for x in neighbors(board, x)]) in [0, 1]:
                set_alive(next, x, False)
            elif reduce(lambda a, b: a+b, [1 if is_alive(board, x) else 0 for x in neighbors(board, x)]) in [2, 3]:
                set_alive(next, x, True)
            elif reduce(lambda a, b: a+b, [1 if is_alive(board, x) else 0 for x in neighbors(board, x)]) >= 4:
                set_alive(next, x, False)
        elif reduce(lambda a, b: a+b, [1 if is_alive(board, x) else 0 for x in neighbors(board, x)]) == 3:
            set_alive(next, x, True)
