...
def nearest_location(loc, loc_list, loc_type=None):
    ...
    return loc_list[[distance(x, loc) if x != None else None for x in map(lambda x: None if x == loc else x, map(lambda x: x if loc_type == None else x if type(x) == loc_type else None, loc_list))].index(min([x for x in [distance(x, loc) if x != None else None for x in map(lambda x: None if x == loc else x, map(lambda x: x if loc_type == None else x if type(x) == loc_type else None, loc_list))] if x != None]))]
...
def pick_cherries_onebyone():
    ...
    """
    """ SYMPLHRWSTE TA KENA APO KATW."""
    cherry_field = ['cherry1', ['cherry2', [
        'cherry3', ['cherry4', ['last cherry', None]]]]]
...
def all_rec(func, ls):
    ...
    if isinstance(ls, int):
        return True if func(ls) else False
    else:
        return all_rec(func, ls[0]) and all_rec(func, ls[1:] if len(ls)>1 else ls[0])
...
def all_lc(func, ls):
    ...
    if [func(x) for x in ls] == [True]*len(ls):
        return True
    else:
        return False
...
def all_hof(func, ls):
    ...
    if list(map(func, ls)) == [True]*len(ls):
        return True
    else:
        return False
...
def primes_up_to(n):
    ...
    return list(filter(lambda x: True if list(map(lambda y: True if x%y==0 else False, list(range(2, int(x**(1/2)) + 1)))) ==  [False]*len(list(range(2, int(x**(1/2)) + 1))) or x==2 else False, list(range(2, n+1))))
